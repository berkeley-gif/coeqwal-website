...
import { Map, useMap, useMapSources, useMapLayers, NavigationControl, GeolocateControl } from "@repo/map"
import {
  strategies,
  hydroclimateOptions,
} from "../../lib/scenarios"
import { OUTCOMES, outcomeDefinitions, outcomeTierValues } from "../../lib/outcomes"
import { useWorkflowStore } from "@repo/state"
import { motion } from "@repo/motion"


// Outcome tooltip
const OutcomeTooltip = ({ outcome, children }: { outcome: string; children: React.ReactElement }) => {
    ...
    <Box sx={{ 
      ...
      {!showMapView && (
        <>
          ...
          <Box sx={{ gridColumn: "span 6", display: "flex", alignItems: "baseline", gap: theme.spacing(theme.cards.spacing.standard) }}>
            <Typography variant="subtitle2">
              Key outcomes
            </Typography>
            <Typography 
      ...
      {/* Strategy rows */}
      {strategies
        .filter(strategy => showOnlyChosen ? chosenStrategies.includes(strategy.value) : true)
        .map((strategy) => (
        <Box 
          key={strategy.value}
          sx={{
        ...
        >
          ...
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              <path 
                ...
                strokeWidth="2" 
                fill={chosenStrategies.includes(strategy.value) ? theme.palette.blue.bright : "none"}
              />
            </svg>
          </Box>
          ...
          <Box sx={{ gridColumn: "span 1", display: "flex", gap: 1, alignItems: "center" }}>
              ...
              <OutcomeTooltip key={outcome} outcome={outcome}>
                <Box 
                  sx={{ 
                    ...
                    border: selectedOutcomes[strategy.value] === outcome && showMapView ? `2px solid ${theme.palette.blue.bright}` : "2px solid transparent",
                    "&:hover": {
                      backgroundColor: showMapView ? theme.palette.grey[100] : "transparent",
                    },
                  }}
                  onClick={showMapView ? () => onOutcomeSelect(strategy.value, outcome) : undefined}
                >
                  <ScenarioGlyph
                    variant="bars"
                    values={[
...
export default function ScenarioExplorer3() {
  ...
  const [selectedOutcomes, setSelectedOutcomes] = useState<Record<string, string | null>>({})
  ...
  const anySelectedOutcome = Object.values(selectedOutcomes).find(outcome => outcome !== null)
  ...
  const handleOutcomeSelect = useCallback((strategyValue: string, outcome: string) => {
    ...
    if (showMapView) {
      ...
      const extentData = OUTCOME_EXTENTS[outcome as keyof typeof OUTCOME_EXTENTS]
      if (extentData) {
        flyTo({
  ...
  return (
    ...
    >
      <DashboardGrid spacing={theme.spacing(theme.cards.spacing.standard)}>
        ...
        >
          ...
          >
            ...
            >
              ...
              {!showMapView && (
                <Box sx={{ 
                  ...
                  <SectionHeader 
                    onClick={() => setStep(currentStep === "choose" ? "none" : "choose")}
                    sx={{ 
                      letterSpacing: 0.5,
                      cursor: "pointer",
                    }}
                  ...
                  {currentStep === "choose" && (
                    <Box sx={{ display: "flex", gap: 3, alignItems: "baseline" }}>
                      ...
                      >
                        {showOnlyChosen ? "Show all strategies" : "Show only chosen strategies"}
                      </Typography>
              ...
              {currentStep === "choose" && (
                <Box sx={{ flex: 1, overflow: "auto" }}>
                  {showMapView ? (
                    /* Map View - Full Card */
                    <Box sx={{ position: "relative", height: "calc(100vh - 48px)", width: "100%", flex: 1 }}>
                      <Map
                      ...
                      <Box sx={{
                        position: "absolute",
                        bottom: theme.spacing(theme.cards.spacing.standard), // Position clear of navigation controls
                      ...
                      <Box sx={{
                        ...
                        <Box sx={{ 
                          ...
                          {/* Left side controls */}
                          <Box sx={{ gridColumn: "1 / 7", display: "flex", alignItems: "baseline", gap: theme.spacing(theme.cards.spacing.standard) }}>
                            <Typography variant="subtitle2">
                              Strategy
                            </Typography>
                            <Typography 
                              ...
                              <FormControlLabel
                                ...
                                sx={{
                                  "& .MuiFormControlLabel-label": {
                                    ...
                                    color: option.value === "historical" ? theme.palette.text.primary : `${theme.palette.grey[500]} !important`,
                                  },
            ...
            >
              <SectionHeader 
                onClick={() => setStep(currentStep === "compare" ? "none" : "compare")}
                sx={{ 
                  letterSpacing: 0.5,
              ...
              {currentStep === "compare" && (
                ...
                <Box
                  ...
                  onClick={() => setStep(currentStep === "empower" ? "none" : "empower")}
                >
                  <SectionHeader 
                    sx={{ 
                      letterSpacing: 0.5,
