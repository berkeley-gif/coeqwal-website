...
/**
 * ReservoirPercentilesSection - Monthly percentile distribution charts
 *
 ...
 *
 * Integrated into the "Reservoir storage" accordion in CategoryView.
 */

import React from "react"
import { Box, Typography, useTheme, CircularProgress, Alert } from "@repo/ui/mui"
...
interface ReservoirPercentilesSectionProps {
  /** Array of selected scenario IDs */
  scenarios: string[]
}
...
/**
 * ReservoirPercentilesSection - Section within reservoir-storage accordion
 */
export default function ReservoirPercentilesSection({
  ...
  if (isLoading && Object.keys(dataByReservoir).length === 0) {
    return (
      ...
      >
        ...
        >
          Loading reservoir percentile data...
        </Typography>
      </Box>
    )
  }
  ...
  if (error) {
    return (
      <Alert severity="warning" sx={{ mb: theme.space.component.lg }}>
        Could not load reservoir percentile data: {error}
      </Alert>
    )
  }
  ...
  return (
    <Box>
      ...
      >
        ...
        <strong>{scenarios.length}</strong> scenario
