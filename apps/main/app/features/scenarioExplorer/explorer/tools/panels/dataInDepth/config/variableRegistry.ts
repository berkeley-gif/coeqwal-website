...
export const VARIABLES: Record<string, VariableDef> = {
  ...
  cws_del_short: {
    ...
    name: "Surface water delivery shortage",
    sectorId: "cwsS",
    locationGroup: "cws",
    unit: "%",
    ...
    views: ["dist"],
    plain:
      "The share of a community water system's estimated potable demand that surface water deliveries did not cover in a year. Zero means demand was fully met. This is the gap the Community deliveries key outcome scores.",
    tech: "Derived on the site from the cws data-in-depth endpoint's pct_demand_met measure (demand-weighted at the NOD_CWS/SOD_CWS aggregates, capped at 100 upstream) on the 74 community water systems with modeled deliveries: shortage percent is 100 minus percent of demand met, so it is never negative. This is the annual surface water delivery shortage of the Content Summary sheet, distinct from the M&I supply shortage of the CWS economics analysis. The served series is aggregated by calendar year over a model run from October 1921 to September 2021, so its first and last years are stubs; the site keeps 1922 to 2020. Water-year-type filtering does not apply: the CWS team aggregated these series by calendar year, not water year.",
    tierOutcome: "CWS_DEL",
    tierOutcomeName: "Community surface water",
    data: "live",
    wytApplicable: false,
  ...
  cws_short: {
    id: "cws_short",
    proseName: "M&I supply shortages",
    name: "M&I supply shortage",
    sectorId: "cwsS",
    ...
    viewUnits: { pct_demand: { unit: "%", unitLabel: "percent of demand" } },
    plain:
      "How much municipal and industrial (M&I) water supply community systems were short of their needs each year, from the CWS economics analysis; the same shortage series that drives welfare loss.",
    tech: "Served live from the cws data-in-depth endpoint on the NOD_CWS/SOD_CWS aggregates and on the 63 community water systems in the CWS economics (welfare) analysis: the volume view reads the shortage_total measure (annual TAF), the percent view reads the served shortage_pct measure directly (0-100, shortage over supply plus shortage). This is the M&I supply shortage of the Content Summary sheet, not the surface water delivery shortage, which is its own variable derived from the delivery source. The shortage set overlaps but differs from the 74-system delivery set, so the location list here is its own. Water-year-type filtering does not apply: the CWS team aggregated these series by calendar year, not water year.",
    data: "live",
    wytApplicable: false,
    mockKind: "short",
    mockEffect: "cwsShort",
