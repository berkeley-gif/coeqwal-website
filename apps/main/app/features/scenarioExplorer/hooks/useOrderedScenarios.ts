 ...
 * the ScenarioSelectionSidebar and ListView. Keeps both panels in lockstep
 * so rows can be aligned pixel-perfectly.
 *
 * Fetches its own score data via useMultipleScenarioTiers (SWR-cached, so
 * multiple callers share the same request).
 *
 * Sort/filter state lives in the Zustand store and persists across tool
 * switches (list → radar → equity, etc.).
 */
