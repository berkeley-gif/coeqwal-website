...
const BaselinePanel = forwardRef(function BaselinePanel(
  { onFlyTo, onLearnMoreClick }: BaselinePanelProps,
  ref,
) {
  ...
  if (!isLoading) {
    try {
      const BaselinePanel = messages.BaselinePanel
      if (
        BaselinePanel &&
        typeof BaselinePanel === "object" &&
        "paragraphs" in BaselinePanel
      ) {
        const paragraphs = BaselinePanel.paragraphs
