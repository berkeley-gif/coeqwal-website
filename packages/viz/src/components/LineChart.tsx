    ...
    const updateChart = useCallback(
      (w: number, h: number) => {
        ...
        const maxValue = Math.max(
          max(processedData, (d) => d.overall) || 0,
          max(processedData, (d) => d.dry) || 0,
          max(processedData, (d) => d.wet) || 0,
        )
        ...
        const lineGenerator = line<{ waterYearIndex: number; value: number }>()
          .x((d) => xScale(d.waterYearIndex))
          .y((d) => yScale(d.value))
          .curve(curveMonotoneX) // Smoothed line
