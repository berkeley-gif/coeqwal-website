...
const PercentileMatrix: React.FC<PercentileMatrixProps> = React.memo(
  ({
    reservoirs,
    scenarios,
    scenarioNames,
      ...
      (w: number, h: number): string | undefined => {
        ...
        if (displayMode === "volume") {
          if (volumeScaleMode === "relative") {
                    ...
                    : maxValue > 10
                      ...
                      : maxValue > 1
                        ...
                        : maxValue > 0.1
                          ...
                          : 0.01
              const rawMaxY =
                Math.ceil((maxValue * 1.05) / increment) * increment
            ...
            sharedYDomain = [
              0,
              yAxisMax !== undefined ? yAxisMax : absComputedMax,
            ]
          }
        }
        ...
        const xScale = scaleLinear().domain([0, 11]).range([0, chartWidth])
            ...
            if (contextNote) {
              ...
              g.append("rect")
                ...
                .on("mouseenter", (event) => {
                  ...
                  let tooltipEl = select<HTMLDivElement, unknown>(
                    "#context-tooltip",
                  )
                  if (tooltipEl.empty()) {
                ...
                .on("mouseleave", () => {
                  select<HTMLDivElement, unknown>("#context-tooltip").style(
                    "visibility",
                    "hidden",
                  )
                })
        ...
        reservoirs.forEach((reservoir, rowIndex) => {
          ...
          const yAxisTicks =
            displayMode === "percentage"
              ? [0, 20, 40, 60, 80, 100]
              : ticks(reservoirYDomain[0], reservoirYDomain[1], 5)
            ...
            if (components && componentData && components.length >= 2) {
              ...
              if (bottomData && topData) {
                ...
                if (overlayData.length > 0) {
                  ...
                  const totalArea = area<OverlayDataItem>()
                  ...
                  const totalMedianLine = line<OverlayDataItem>()
                    .x((d) => xScale(d.monthIndex))
                  ...
                  const mwdArea = area<OverlayDataItem>()
                    .x((d) => xScale(d.monthIndex))
                    .y0(() => cellYScale(0))
                  ...
                  const mwdMedianLine = line<OverlayDataItem>()
                    .x((d) => xScale(d.monthIndex))
          ...
          reservoirs.forEach((reservoir, rowIndex) => {
            scenarios.forEach((scenarioId, colIndex) => {
              ...
              if (stats.hasDryYearMonths) {
                ...
                // Add dotted underline
                const dryYearTextWidth =
                  dryYearText.node()?.getBBox().width ?? 85
                statsG
                  .append("line")
                ...
                statsG
                  ...
                  .on("mouseleave", () => {
                    select<HTMLDivElement, unknown>("#dry-year-tooltip").style(
                      "visibility",
                      "hidden",
                    )
                  })
              ...
              if (reservoir.reservoirId === "cvp_sod") {
                ...
                const shortageYears = Math.round(
                  stats.shortageFrequencyPct ?? 0,
                )
              ...
              statsG
                ...
                .on("mouseleave", () => {
                  select<HTMLDivElement, unknown>("#cws-context-tooltip").style(
                    "visibility",
                    "hidden",
                  )
                })
            ...
            g.append("rect")
              ...
              .on("mousemove", (event) => {
                ...
                if (monthIndex >= 0 && monthIndex < 12) {
                  ...
                  const playheadX =
                    cellX + chartPadding.left + xScale(monthIndex)
                  playhead
                    .attr("x1", playheadX)
                    .attr("x2", playheadX)
                    .attr("y1", cellTop)
...
PercentileMatrix.displayName = "PercentileMatrix"
