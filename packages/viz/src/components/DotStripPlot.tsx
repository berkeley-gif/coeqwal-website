...
const DotStripPlot: React.FC<DotStripPlotProps> = React.memo(
  ({
    data,
    axes,
    baselineData,
    ...
    const updateChart = useCallback(
      (w: number, h: number) => {
        ...
        // Horizontal guide line per row (at center of band)
        axes.forEach((axis) => {
        ...
        // Vertical jitter within each band to reduce overlap
        const nonBaselineData = data.filter(
          (s) => !baselineData || s.id !== baselineData.id,
        )
        const scenarioCount = nonBaselineData.length
        const jitterOffset = (index: number) => {
          if (scenarioCount <= 1) return 0
          const spread = bandH * 0.5
        ...
        nonBaselineData.forEach((scenario, si) => {
          ...
          axes.forEach((axis) => {
            ...
            const cx = xScale(sv)
            const cy = yScale(axis)! + bandH / 2 + jitterOffset(si)
...
DotStripPlot.displayName = "DotStripPlot"

export default DotStripPlot
