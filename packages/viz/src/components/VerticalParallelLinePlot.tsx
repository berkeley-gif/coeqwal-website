        ...
        (g: Selection<SVGGElement, unknown, null, undefined>) => {
          ...
          data.forEach((scenario, scenarioIndex) => {
            ...
            axes.forEach((axisName) => {
              g.select(
                `.circle-${scenarioIndex}-${axisName.replace(/\s+/g, "-")}`,
              )
                .transition()
                .duration(300)
                .ease(easeQuadOut)
      ...
      const updateChart = useCallback(
        (newWidth: number, newHeight: number, animate = true) => {
          ...
          currentAxes.forEach((axis) => {
            scales[axis] = scaleLinear().domain([-1, 1]).range([0, innerWidth])
          })
          ...
          currentAxes.forEach((axis, axisIndex) => {
            ...
            const lineSelection = animate
              ? axisLine.transition(t as any) // eslint-disable-line @typescript-eslint/no-explicit-any
              : axisLine
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            const dragBehavior = drag<SVGGElement, unknown>()
              .on("start", function () {
                ...
                select(this.parentNode as SVGGElement).style("opacity", 0.7)
              })
              ...
              .on("end", function () {
                ...
                select(this.parentNode as SVGGElement).style("opacity", 1)
                select(this.parentNode as SVGGElement)
                  .select(".drag-handle")
                  .style("cursor", "grab")
            ...
            const rightArrowData = [
              { type: "right", position: currentFilter[1] },
            ]
            ...
            const hasActiveFilter =
              currentFilter[0] > -1 || currentFilter[1] < 1
            if (hasActiveFilter) {
              axisGroup
                .append("line")
          ...
          if (showBaseline && baselineData) {
            ...
            const baselineLineGenerator = line<[string, number]>()
            ...
            currentAxes.forEach((axis) => {
              ...
              const circleSelection = animate
                ? circle.transition(t as any) // eslint-disable-line @typescript-eslint/no-explicit-any
                : circle
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          data.forEach((d, dataIndex) => {
            ...
            const lineGenerator = line<[string, number | undefined]>()
...
VerticalParallelLinePlot.displayName = "VerticalParallelLinePlot"
