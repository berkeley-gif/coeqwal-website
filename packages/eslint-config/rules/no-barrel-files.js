...
export const noBarrelFilesRule = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Disallow barrel files (index files that re-export multiple modules)',
      ...
      Program(node) {
        ...
        if (/[/\\]index\.(js|ts|jsx|tsx|mjs|mts)$/.test(filename)) {
          ...
          if (exportStatements.length > 1) {
            context.report({
              node,
              message: 'Barrel files with multiple exports are not allowed. Use explicit imports and exports instead.'
            });
          }
        }
      }
