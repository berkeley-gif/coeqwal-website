...
Exported from `@repo/data/coeqwal/hooks` (and `@repo/data/fetching` for `useLocalData`). Grouped by domain, paired with the cache key each one owns.

| Domain          | Hook                                                                                                                                                                                                           | Cache key it reads                                  |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| Tier metadata   | `useTiers()`                                                                                                                                                                                                   | `TIER_LIST`                                         |
| Tier metadata   | `useTierMapping()`                                                                                                                                                                                             | derived from `useTiers`                             |
| Scenarios       | `useScenarios()`                                                                                                                                                                                               | `SCENARIOS`                                         |
| Tier scores     | `useScenarioTiers(id)`                                                                                                                                                                                         | `scenarioTiers(id)`                                 |
| Tier scores     | `useScenarioTierByCode(id, code)`                                                                                                                                                                              | `scenarioTierByCode(id, code)`                      |
| Tier locations  | `useTierLocationAssignments(id, code)`                                                                                                                                                                         | `tierLocations(id, code)`                           |
| Tier locations  | `useTierLocationAssignmentsBatch(id, codes)`                                                                                                                                                                   | `tierLocationsBatch(id, codes)`                     |
| Reservoirs      | `useReservoirList`, `useAllReservoirsList`, `useScenariosWithPercentiles`, `useReservoirPercentiles`, `useAllReservoirPercentiles`, `useGroupedReservoirPercentiles`, `useStorageMonthly`, `useSpillMonthly`, `useMultipleReservoirPercentiles`, `useReservoirPeriodSummary` | corresponding reservoir keys                        |
| CWS / M&I       | `useCwsAggregatesList`, `useCwsAggregatesMonthly`, `useCwsAggregatesPeriod`, `useMiContractorsList`, `useMiContractorsMonthly`, `useMiContractorsPeriod`                                                       | corresponding CWS / M&I keys                        |
| Urban demand    | `useDemandUnitsList`, `useDemandUnitsGroups`, `useDemandUnitStatistics`, `useDemandUnitsMonthly`, `useDemandUnitsPeriod`                                                                                       | corresponding demand-unit keys                      |
| AG              | `useAgAggregatesMonthly`, `useAgAggregatesPeriod`, `useAgDemandUnitsList`, `useAgDemandUnitsDeliveryMonthly`, `useAgDemandUnitsShortageMonthly`, `useAgDemandUnitsPeriod`                                       | corresponding AG keys                               |
| Refuge          | `useRefugeDemandUnitsList`, `useRefugeDusDeliveryMonthly`, `useRefugeDusShortageMonthly`, `useRefugeDusPeriod`                                                                                                 | corresponding refuge keys                           |
| Channels        | `useChannelsList`, `useEnvFlowSeasons`, `useChannelsMonthly`, `useChannelsSeasonal`, `useChannelsPeriodSummary`                                                                                                | corresponding channels keys                         |
| Delta           | `useDeltaMonthly(id, category?)`                                                                                                                                                                               | `deltaMonthly(id, category?)`                       |
| Batch stats     | `useBatchStatistics(scenarios, types?)` + selectors `getStorageForScenario`, `getCwsForScenario`, `getAgForScenario`, `getEnvFlowForScenario`                                                                  | `batchStatistics(...)`                              |
| Local JSON      | `useLocalData(url, options?)`                                                                                                                                                                                  | URL or custom cache key                             |

Every hook returns the same envelope: `{ <data property>, isLoading, error }`. The data property name varies (`tiers`, `scenarios`, `data`, `tierMapping`, ...) but `isLoading` and `error` are always there.
...
| `apps/main/app/features/scenarioExplorer/tools/panels/resilience/useResilienceLoiSensitivity.ts`  | Resilience LOI sensitivity panel mounts             | `tierLocationsBatch(mapped, codes)` for each (scenario, climate) cell                                                                        |
...
- **Individually added demand units** (when the user picks one from the dropdown), via `useIndividualDemandUnitsData` which wraps `fetchDemandUnitStatistics(scenarioId, duId)` in a `useSWR` keyed by `["individual-demand-units", ...scenarios, ...duIds]`
